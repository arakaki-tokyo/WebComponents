...
const dirName = import.meta.url.match(/^.*\//g)[0];

import(`${dirName}../widgets.mjs`).then(w => {
    globalThis.widgets = w;
    customElements.define("w-slider", w.Slider);
});

let pythonInited = false;
async function initPython(exec) {
    if (pythonInited) return;
    pythonInited = true;
    try {
        await exec(`
            import sys
            ...
            with open('./widgets.py', "w") as f:
                f.write(pyodide.open_url('${dirName}widgets.py').read())
...
(function insertStyle() {
    ...
    style.innerHTML =
        ...
        `
        [data-role="out"] table.dataframe{
            border-collapse: collapse;
            border: none;
            font-size: 12px;
            border-color: transparent;
        }
        [data-role="out"] .dataframe thead{
            border-bottom: 1px solid gray;
        }
        [data-role="out"] .dataframe th, 
        [data-role="out"] .dataframe tr, 
        [data-role="out"] .dataframe td{
            text-align: right;
            padding: 0.5em 0.5em;
        }
        [data-role="out"] .dataframe tbody tr:nth-child(odd) {
            background: whitesmoke;
        }
        [data-role="out"] .dataframe tbody tr:hover {
...
class Stretchable extends HTMLElement {
    #isVisible = true;
    connectedCallback() {
        ...
        shadow.innerHTML = `
            ...
            <div>
                <svg data-role="extend" fill="lightgray" width=30 viewBox="0 0 8 2">
        ...
        if ("visible" in this.attributes) {
            const initView = this.getAttribute("visible");
            if (initView.match(/^$|true/g)) {
                this.isVisible = true;
...
customElements.define("stretch-able", Stretchable);
/**
...
export class PyCell extends HTMLElement {
    ...
    async connectedCallback() {
        ...
        this.innerHTML = `
            ...
            <stretch-able data-role="out"></stretch-able>
        `;
        ...
        if ((Function(`try{return typeof ${this.env.pyodide}}catch(e){return "undefined"}`)()) !== "undefined") {
            ...
            if (typeof (this.Pyodide) === "object") {
                const p = await this.Pyodide;
                initPython(p.runPythonAsync);
            } else {
    ...
    btnOnclick(e) {
        ...
        this.execCode(this.input.value, this.out, this, e);
